...
with st.sidebar:
    new_lang = st.selectbox("Language / 语言", ["zh", "en"], index=0 if get_lang() == "zh" else 1, format_func=lambda x: t("lang_zh") if x == "zh" else t("lang_en"), key="lang_ql")
    ...
    gamma = st.slider(t("gamma"), 0.0, 0.99, 0.0, 0.01, help=t("gamma_help"))
    epsilon = st.slider(t("epsilon"), 0.0, 0.5, 0.1, 0.01)
    ...
    st.markdown("---")
    run_clicked = st.button(t("btn_run_ql"), type="primary")
    if st.session_state.get("ql_result"):
        if st.button(t("btn_rerun"), key="ql_rerun"):
...
if st.session_state.get("ql_result"):
    ...
    with row_btn:
        if st.button(t("btn_submit_run"), key="ql_submit"):
            add_submission(
                "Qlearning",
                get_user_id(),
                get_nickname(),
